...
# Expose audio mount in the event YAIS is configured to save
app.mount(
...
@app.post(
    "/api/yais",
    response_model=ASR,
    summary="Stream YAIS audio for ASR",
    response_description="ASR engine output",
)
async def yais(
    request: Request,
    ...
    logger.debug(
        f"FASTAPI: Got YAIS request for model {model} beam size {beam_size} "
        f"language detection {detect_language}"
    )
    task = "transcribe"
    ...
    if force_language:
        if not check_language(force_language):
            logger.debug(
                "YAIS: Invalid force_language in request - returning HTTP 400"
            )
            response = {"error": "Invalid force_language"}
            return JSONResponse(
                content=response, status_code=status.HTTP_400_BAD_REQUEST
    ...
    yais_id = request.headers.get("x-yais-id", "").lower()

    logger.debug(
        f"YAIS: Audio information: sample rate: {sample_rate}, bits: {bits}, channel(s): {channel}, "
        f"codec: {codec}"
    )

    if yais_id:
        logger.debug(f"YAIS: Got YAIS ID {yais_id}")
    ...
    try:
        if codec == "pcm":
            logger.debug("YAIS: Source audio is raw PCM, creating WAV container")
            audio_file = write_stream_wav(
                body, int(sample_rate), int(bits), int(channel)
            )
        elif codec == "wav":
            logger.debug("YAIS: Source audio is wav")
            audio_file = io.BytesIO(body)
            audio_file.seek(0)
        else:
            logger.debug(f"YAIS: Converting {codec} to wav")
            file = io.BytesIO(body)
            file.seek(0)
            audio_file = audio_to_wav(file, int(sample_rate))
    except Exception:
        logger.debug("YAIS: Invalid audio in request - returning HTTP 400")
        response = {"error": "Invalid audio"}
        return JSONResponse(content=response, status_code=status.HTTP_400_BAD_REQUEST)

    # Save received audio if requested - defaults to false
    if save_audio:
        save_filename = "nginx/static/audio/yais.wav"
        logger.debug(f"YAIS: Saving audio to {save_filename}")
    ...
    if voice_auth:
        ...
        if sv_results:
            logger.debug("YAIS: Authenticated voice")
            # Seek back to 0 for Whisper later
            audio_file.seek(0)
            speaker = list(sv_results.keys())[0]
            speaker_status = f"I heard {speaker} say:"
        else:
            logger.debug("YAIS: Unknown or unauthorized voice - returning HTTP 406")
